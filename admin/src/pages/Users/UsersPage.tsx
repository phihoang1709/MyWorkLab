...
const UsersPage = () => {
    ...
    const {data, error, isLoading, isValidating, mutate} = useSWR(
        `${import.meta.env.VITE_API_URL}/categories?page=${page}`, async () => {
            try {
                const {data} = await axios.get(`${import.meta.env.VITE_API_URL}/categories?page=${page}`);
                return data;
            } catch (error) {
    ...
    },  { refreshInterval: 1000, revalidateOnMount: true, revalidateOnFocus: true, revalidateOnReconnect: true, dedupingInterval: 1000, refreshWhenHidden: true, refreshWhenOffline: true, refreshWhenOnline: true, focusThrottleInterval: 1000, revalidateAll: true }
    ...
    const onDelete = async (id: string) => {
            try {
                ...
                mutate(`${import.meta.env.VITE_API_URL}/categories?page=${page}`)
            } catch (error) {
                console.error(error);
            }
    }
